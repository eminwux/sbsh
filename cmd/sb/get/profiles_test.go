...
func Test_ErrInvalidOutputFormat_Profile_List(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(os.Stderr, &slog.HandlerOptions{Level: slog.LevelDebug}))
	cmd := &cobra.Command{}
	ctx := context.WithValue(context.Background(), types.CtxLogger, logger)
	cmd.SetContext(ctx)

	viper.Set(config.SB_GET_PROFILES_OUTPUT.ViperKey, "bogus")
	defer viper.Set(config.SB_GET_PROFILES_OUTPUT.ViperKey, "")
