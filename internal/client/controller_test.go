...
func Test_ErrNoTerminalSpec(t *testing.T) {
	...
	// With nil TerminalSpec and ClientMode set to AttachToTerminal,
...
func Test_ErrClientMode(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(os.Stderr, &slog.HandlerOptions{Level: slog.LevelDebug}))
	sc := NewClientController(context.Background(), logger).(*Controller)
	sc.NewClientRunner = func(ctx context.Context, logger *slog.Logger, _ *api.ClientDoc, _ chan<- clientrunner.Event) clientrunner.ClientRunner {
		return &clientrunner.Test{
	...
	// Note: Since ClientMode is now an explicit field in ClientSpec, we can't test invalid mode directly.
	// This test is removed as it's no longer applicable with the new architecture.
	// If we need to test invalid scenarios, we should test invalid ClientMode values or TerminalSpec configurations instead.
	t.Skip("Test_ErrClientMode is no longer applicable - ClientMode is now an explicit field")
}
...
func Test_EventCmdExited(t *testing.T) {
	...
	ev := clientrunner.Event{
		ID:   "test-terminal",
		Type: clientrunner.EvCmdExited,
		Err:  nil,
		When: time.Now(),
	}
	sc.eventsCh <- ev
...
func Test_EventUnknown(_ *testing.T) {
	...
	ev := clientrunner.Event{
		ID:   "test-terminal",
		Type: clientrunner.EventType(999), // Unknown event type
		Err:  nil,
		When: time.Now(),
	}
	sc.eventsCh <- ev
...
// Test_ClientAttach_WaitForStartingOrReady verifies that the client attach flow
...
// clientrunner/io_test.go. This test verifies the attach flow completes successfully.
func Test_ClientAttach_WaitForStartingOrReady(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
// Test_ClientAttach_StateTransition verifies the complete attach flow works correctly.
func Test_ClientAttach_StateTransition(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
