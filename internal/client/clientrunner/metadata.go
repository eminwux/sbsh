...
func (sr *Exec) CreateMetadata() error {
	...
	dir := filepath.Join(runPath, defaults.ClientsRunPath, string(sr.id))
	sr.logger.Debug("creating metadata", "runPath", dir)
	if err := os.MkdirAll(dir, 0o700); err != nil {
		sr.logger.Error("failed to create client dir", "dir", dir, "error", err)
...
func (sr *Exec) updateMetadata() error {
	...
	dir := filepath.Join(sr.metadata.Spec.RunPath, defaults.ClientsRunPath, string(sr.id))
